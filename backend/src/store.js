...
async function createRoom(ownerId) {
  ...
  return withTransaction(async (client) => {
    ...
    const room = await client.query(
      `INSERT INTO rooms (code, owner_id, status)
       VALUES ($1, $2, 'open')
    ...
    if (existing.rowCount) {
      ...
      await client.query(
        `INSERT INTO room_members
         (room_id, user_id, role, connected)
...
async function shareDraft(roomId, userId, body) {
  ...
  const { rows } = await query(
    `INSERT INTO drafts
     (user_id, room_id, name, duration_ms, storage_url, effect)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING *`,
    [
      userId,
...
async function loadSpinGraph(spinId) {
  ...
  const parts = await query(
    'SELECT * FROM spin_participants WHERE spin_id = $1',
    [spinId]
  );
...
async function persistEngineSpin(
  client,
  spinId,
  engineSpin,
  existingEventCount
  ...
  for (const event of engineSpin.events) {
    ...
    // Add winner name to winner_announced event
    let payload = event.payload;

    if (
      event.type === 'winner_announced' &&
      ...
      const winnerName =
        winnerRes.rows[0]?.display_name || 'Unknown player';
...
async function startSpin(roomId, userId) {
  return withTransaction(async (client) => {
    ...
    if (room.owner_id !== userId) {
      const err = new Error(
        'Only the room owner can start a spin'
      );
      err.status = 403;
      err.code = 'NOT_OWNER';
      throw err;
    ...
    const started = engine.startSpin({
    ...
    const spinIns = await client.query(
      `INSERT INTO spins
       (room_id, status, started_by, started_at,
        next_elimination_at, virtual_points)
...
async function applyLeaveToSpin(roomId, userId) {
  return withTransaction(async (client) => {
    ...
    const engineSpin = dbSpinToEngine(
      spinRow,
      parts.rows
    );
    ...
    const inserted = await persistEngineSpin(
      client,
      spinRow.id,
      engineSpin,
      events.rows[0].n
    ...
    return {
      spinId: spinRow.id,
      inserted,
      status: engineSpin.status
    };
  });
}

async function tickSpin(
  spinId,
  now = new Date(),
  rng = Math.random
) {
  return withTransaction(async (client) => {
    ...
    if (!spinRow || spinRow.status !== 'running') {
      return null;
    }
